...
const DiagnosisSchema = z.object({
    ...
    severity: z.enum(['Low', 'Medium', 'High']).optional().describe('The estimated severity of the issue.'),
});
...
export const AnalyzeImageOutputSchema = z.object({
  ...
  visualObservations: z.array(z.string()).describe('Specific visual findings identified in the image (e.g., specific spot patterns, edge yellowing).'),
  diagnosis: z.object({
  ...
  diseaseInfo: z.object({
      ...
      prevention: z.array(z.string()).describe('Long-term prevention tips to avoid recurrence.'),
  }).optional(),
